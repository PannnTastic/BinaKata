...
export async function POST(req: NextRequest) {
  try {
    const { childId, wordIndex, difficulty, isCorrect, hintsUsed, shufflesUsed } = await req.json();
    ...
    const wordProgress = await prisma.wordProgress.upsert({
      ...
      update: {
        ...
        shufflesUsed: { increment: shufflesUsed || 0 },
      ...
      create: {
        ...
        shufflesUsed: shufflesUsed || 0,
        isCompleted: isCorrect,
        lastAttempt: new Date()
      }
    });

    // Update overall learning progress for words module
    const completedWords = await prisma.wordProgress.count({
      where: {
        childId,
        isCompleted: true
      }
